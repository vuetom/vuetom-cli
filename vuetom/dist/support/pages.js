...
const createRouterRoutes = (loadPageModule, fallbackComponent) => {
    // const router = useRouter()
    const { theme } = useData();
    const { pages } = theme.value;
    let latestPendingPath = '';
    async function loadPage(href = '/index', scrollPosition = 0, isRetry = false) {
...
function pathToFile(path) {
    ...
    if (import.meta.env.DEV) {
        ...
        if (inBrowser) {
            ...
            pagePath = `${pagePath.slice(base.length - 1).replace(/\//g, '_') || 'index'}.md`;
